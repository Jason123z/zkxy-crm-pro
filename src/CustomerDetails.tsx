...
import { Customer, VisitRecord, Contact, Task, CustomerLevel } from './types';
...
export default function CustomerDetails({ customerId, onBack, onShowToast }: CustomerDetailsProps) {
      ...
      if (customerId) {
        try {
          ...
          if (data) {
            ...
            // 异步从后端拉取 Contacts
            const loadedContacts = await getContactsByCustomer(customerId);
            ...
            // 异步从后端拉取 Visits
            const loadedVisits = await getVisitsByCustomer(customerId);
            if(loadedVisits.length > 0) {
                setVisits(loadedVisits);
            }

            // 异步从后端拉取 Tasks
            const loadedTasks = await getTasksByCustomer(customerId);
            if(loadedTasks.length > 0) {
                setTasks(loadedTasks);
            }

            // 根据 status 设置初始阶段
            const currentStage = stages.find(s => s.name === data.status);
            if (currentStage) {
  ...
  const [requirements, setRequirements] = useState({
    product: '企业ERP管理系统 v4.0',
    competitors: '阿里零售通, 顺丰科技',
    description: '目前客户主要面临由于订单量激增导致的调度延迟问题，需要一套能够自动化分配运力并实时监控车辆位置的系统，需支持API与现有的OA系统对接。'
  });
  ...
  const handleStageChange = async (newStageId: number) => {
    ...
    if (customer) {
      ...
      const updatedCustomer = { ...customer, status: stageName };
      setCustomer(updatedCustomer);
      await updateCustomer(customer.id, {status: stageName});
      onShowToast(`销售阶段已更新为：${stageName}`);
    }
  };
  ...
  const handleSaveCustomer = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setContacts([
      {
        ...
        avatar: `https://picsum.photos/seed/${updatedCustomer.id}/100/100`
      }
    ]);
    ...
    try {
        ...
        onShowToast('客户资料已更新');
        onBack(); // Return to list after saving
    } catch(err: any) {
        console.error(err);
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-slate-500 mt-1">行业：{customer.industry} · 规模：{customer.size}</p>
            </div>
          </div>
      ...
      <div className="p-4 space-y-6 max-w-4xl mx-auto w-full">
        ...
        <section className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm text-slate-500">已停留: 12天</span>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <select 
                  ...
                  onChange={(e) => setCustomer({ ...customer, level: e.target.value as CustomerLevel })}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 border-slate-200 rounded-lg p-2.5 text-sm focus:ring-blue-600 focus:border-blue-600 outline-none"
                >
                  <option>企业ERP管理系统 v4.0</option>
                  <option>智能物流调度模块</option>
                  <option>CRM基础云服务</option>
                </select>
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <input 
                  ...
                  type="text" 
                  value={requirements.competitors}
                  onChange={(e) => setRequirements({ ...requirements, competitors: e.target.value })}
                />
              </div>
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {contact.isKey && (
                      <span className="text-[10px] bg-blue-600 text-white px-1.5 py-0.5 rounded uppercase font-bold">Key</span>
                    )}
                  </div>
        ...
        <form onSubmit={handleSaveCustomer} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input name="industry" defaultValue={customer.industry} className="w-full bg-slate-50 border-slate-200 rounded-lg p-2.5 text-sm" />
            </div>
          </div>
          <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-slate-700">职位</label>
        ...
        <form onSubmit={handleSaveVisit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select name="type" defaultValue={editingVisit?.type || "线下拜访"} className="w-full bg-slate-50 border-slate-200 rounded-lg p-2.5 text-sm">
                <option>线下拜访</option>
                <option>电话拜访</option>
                <option>视频会议</option>
                <option>其他</option>
      ...
      >
        <form onSubmit={handleSaveContact} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-semibold text-slate-700">职位/角色</label>
            <input name="role" defaultValue={editingContact?.role} placeholder="例如：技术总监 (决策者)" className="w-full bg-slate-50 border-slate-200 rounded-lg p-2.5 text-sm" required />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <input name="email" type="email" defaultValue={editingContact?.email} className="w-full bg-slate-50 border-slate-200 rounded-lg p-2.5 text-sm" />
            </div>
          </div>
          <div className="flex items-center gap-2 py-2">
            <input name="isKey" type="checkbox" defaultChecked={editingContact?.isKey} className="rounded border-slate-300 text-blue-600 focus:ring-blue-600" />
            <label className="text-sm font-semibold text-slate-700">设为核心决策人 (Key)</label>
          </div>
          <div className="flex gap-3 pt-4">
