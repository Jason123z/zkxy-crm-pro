...
export default function CustomerList({ onBack, onSelectCustomer, onNavigate, onShowToast }: CustomerListProps) {
  ...
  const stages = ['全部', '线索', '初步拜访', '需求调研', '询价', '合同'];

  const getStatusColor = (status: string) => {
    switch (status) {
      case '线索': return 'bg-slate-100 text-slate-600';
      case '初步拜访': return 'bg-emerald-100 text-emerald-800';
      case '需求调研': return 'bg-blue-100 text-blue-800';
      case '询价': return 'bg-amber-100 text-amber-800';
      case '合同': return 'bg-purple-100 text-purple-800';
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 pb-24">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-10 flex border-t border-slate-200 bg-white px-4 pb-6 pt-2">
        ...
        <button className="flex flex-1 flex-col items-center justify-center gap-1 text-blue-600">
          <Users size={24} />
          <span className="text-[10px] font-medium">客户</span>
        </button>
        <button onClick={() => onNavigate('report')} className="flex flex-1 flex-col items-center justify-center gap-1 text-slate-400">
          <FileText size={24} />
