...
export default function Profile({ onNavigate, onShowToast }: ProfileProps) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      if (sbUser) {
        // 合并 Supabase Auth 数据和本地 Mock 数据（作为过渡）
        const profile = getUserProfile();
        setUser({
          ...profile,
          email: sbUser.email || profile.email,
          name: sbUser.user_metadata?.full_name || profile.name,
        });
      }
    };
    fetchUser();
  ...
  return (
    <div className="flex flex-col min-h-screen w-full max-w-md mx-auto bg-white shadow-xl pb-24">
      ...
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white border-t border-slate-100 flex items-center justify-around py-3 px-2 z-50">
        <button onClick={() => onNavigate('dashboard')} className="flex flex-col items-center gap-1 text-slate-400">
          <Home size={24} />
          <span className="text-[10px] font-medium">首页</span>
        </button>
        <button onClick={() => onNavigate('customers')} className="flex flex-col items-center gap-1 text-slate-400">
          <Users size={24} />
          <span className="text-[10px] font-medium">客户</span>
        </button>
        <button onClick={() => onNavigate('report')} className="flex flex-col items-center gap-1 text-slate-400">
          <FileText size={24} />
