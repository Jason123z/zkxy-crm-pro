...
class CustomerService:
    ...
    def get_projects_by_customer(self, customer_id: str, user_id: str) -> List[dict]:
        response = self.db.table("projects").select("*").eq("customer_id", customer_id).eq("user_id", user_id).order("created_at", desc=True).execute()
        return response.data
    ...
    def update_project(self, project_id: str, project: ProjectUpdate, user_id: str) -> dict:
        response = self.db.table("projects").update(project.model_dump(exclude_unset=True)).eq("id", project_id).eq("user_id", user_id).execute()
        return response.data[0]
    ...
    def get_contacts_by_customer(self, customer_id: str, user_id: str, project_id: Optional[str] = None) -> List[dict]:
        query = self.db.table("contacts").select("*").eq("customer_id", customer_id).eq("user_id", user_id)
        if project_id:
    ...
    # --- Visit Records ---
    def get_visits_by_customer(self, customer_id: str, user_id: str, project_id: Optional[str] = None) -> List[dict]:
        query = self.db.table("visit_records").select("*").eq("customer_id", customer_id).eq("user_id", user_id)
        if project_id:
            query = query.eq("project_id", project_id)
    ...
    def get_tasks_by_customer(self, customer_id: str, user_id: str, project_id: Optional[str] = None) -> List[dict]:
        query = self.db.table("tasks").select("*").eq("customer_id", customer_id).eq("user_id", user_id)
        if project_id:
            query = query.eq("project_id", project_id)
        response = query.order("created_at", desc=True).execute()
        return response.data
